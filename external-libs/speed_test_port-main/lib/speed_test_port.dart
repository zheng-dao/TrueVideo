...
class SpeedTest {
  ...
  Future<Settings> GetSettings() async {
    ...
    Settings settings = Settings.fromXMLElement(XmlDocument.from(response.body)?.getElement('settings'));
    ...
    for (var element in ServersUrls) {
      ...
      try {
        ...
        serversConfig = ServersList.fromXMLElement(XmlDocument.from(resp.body)?.getElement('settings'));
      } catch (ex) {
  ...
  Uri CreateTestUrl(Server server, String file) {
    return Uri.parse(Uri.parse(server.Url).toString().replaceAll('upload.php', file));
  }
  ...
  List<String> GenerateDownloadUrls(Server server, int retryCount) {
    ...
    DownloadSizes.forEach((downloadSize) {
      for (var i = 0; i < retryCount; i++) {
        result.add(downloadUriBase.toString().replaceAll('%7B0%7D', downloadSize.toString()).replaceAll('%7B1%7D', i.toString()));
      }
    });
  ...
  Future<double> TestDownloadSpeed(
    ...
    testData.forEach((element) {
      tasks.add(Future<int>(() async {
        ...
        try {
          ...
          sum += data.bodyBytes.length;
          final speed = ((sum * 8 / 1024) / (stopwatch.elapsedMilliseconds / 1000) / 1000);
          callback?.call(speed, count / testData.length);
          count++;
    ...
    return (totalSize * 8 / 1024) / (stopwatch.elapsedMilliseconds / 1000) / 1000;
  }
    ...
    testData.forEach((element) {
      tasks.add(Future<int>(() async {
        ...
        try {
          ...
          await http.post(Uri.parse(server.Url), body: element);
  ...
  List<String> GenerateUploadData(int retryCount) {
    ...
    for (var sizeCounter = 1; sizeCounter < MaxUploadSize + 1; sizeCounter++) {
      ...
      for (var i = 0; i < size; ++i) builder.write(Chars[random.nextInt(Chars.length)]);
...
class Client {
  ...
  Client.fromXMLElement(XmlElement? element)
      : this.Ip = element?.getAttribute('ip') ?? "",
        ...
        this.geoCoordinate = Coordinate(double.parse(element?.getAttribute('lat') ?? ""), double.parse(element?.getAttribute('lon') ?? ""));
}
