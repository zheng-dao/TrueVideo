...
@property(strong, nonatomic) NSString *microphone;
@end
...
- (instancetype)initWithCameraName:(NSString *)cameraName
                  resolutionPreset:(NSString *)resolutionPreset
                       ...
                       orientation:(UIDeviceOrientation)orientation
                        microphone:(NSString *)microphone
...
- (void)setupMicrophone {
    NSLog(@"Create camera %@", _microphone);
    ...
    {
       NSLog(@"Microphone port %@", port.portName);
    }
    ...
    for (AVAudioSessionPortDescription* port in inputs)
    {
        ...
        {
            builtInMicPort = port;
            NSLog(@"Built in mic port %@", port.portName);
            break;
        }
    }
    ...
    if(builtInMicPort){
        ...
        // Search data source from builtInMicPort
        AVAudioSessionDataSourceDescription* dataSource = nil;
        for (AVAudioSessionDataSourceDescription* source in builtInMicPort.dataSources)
        {
            NSLog(@"Data source orientation %@", source.orientation);
            if ([[source.orientation lowercaseString] isEqualToString:_microphone])
            {
                dataSource = source;
                NSLog(@"Data source found %@", source.orientation);
                break;
            }
        }

        if(dataSource){
            NSLog(@"Currently selected source is \"%@\" for port \"%@\"", builtInMicPort.selectedDataSource.orientation, builtInMicPort.portName);
            NSLog(@"Attempting to select source \"%@\" on port \"%@\"", dataSource.orientation, builtInMicPort.portName);

            NSError* error = nil;
            [builtInMicPort setPreferredDataSource:dataSource error:&error];
            [myAudioSession
            ...
            if (error)
            {
                NSLog(@"Change mic error failed %@", error);
            }else{
    ...
    if(!changed){
        _captureSession.automaticallyConfiguresApplicationAudioSession = TRUE;
    }
}
