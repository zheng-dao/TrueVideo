...
class CameraController extends ValueNotifier<CameraValue> {
  ...
  // Wich audio source use on Android
  final int androidAudioSource;

  // Wich microphone use on iOS
  final String iosMicrophone;
  ...
  static Future<void> resetAudio() async {
    await CameraPlatform.instance.resetAudio();
  }
  ...
  Future<void> initialize() async {
    ...
    try {
      final Completer<CameraInitializedEvent> _initializeCompleter = Completer<CameraInitializedEvent>();
      ...
      value = value.copyWith(
        ...
        exposureMode: await _initializeCompleter.future.then((CameraInitializedEvent event) => event.exposureMode),
        focusMode: await _initializeCompleter.future.then((CameraInitializedEvent event) => event.focusMode),
        exposurePointSupported: await _initializeCompleter.future.then((CameraInitializedEvent event) => event.exposurePointSupported),
        focusPointSupported: await _initializeCompleter.future.then((CameraInitializedEvent event) => event.focusPointSupported),
      );
  ...
  Future<void> pausePreview() async {
    ...
    try {
      ...
      value = value.copyWith(isPreviewPaused: true, previewPauseOrientation: Optional<DeviceOrientation>.of(value.deviceOrientation));
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  }
  ...
  Future<void> startImageStream(onLatestImageAvailable onAvailable) async {
    ...
    _imageStreamSubscription = cameraEventChannel.receiveBroadcastStream().listen(
      (dynamic imageData) {
        ...
        onAvailable(CameraImage.fromPlatformData(imageData as Map<dynamic, dynamic>));
      },
    );
  }
  ...
  Future<void> startVideoRecording() async {
    ...
    try {
      ...
      value = value.copyWith(
          ...
          isRecordingPaused: false,
          recordingOrientation: Optional<DeviceOrientation>.fromNullable(value.lockedCaptureOrientation ?? value.deviceOrientation));
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  }
  ...
  Future<void> lockCaptureOrientation([DeviceOrientation? orientation]) async {
    try {
      ...
      value = value.copyWith(lockedCaptureOrientation: Optional<DeviceOrientation>.fromNullable(orientation ?? value.deviceOrientation));
    } on PlatformException catch (e) {
      throw CameraException(e.code, e.message);
    }
  }
