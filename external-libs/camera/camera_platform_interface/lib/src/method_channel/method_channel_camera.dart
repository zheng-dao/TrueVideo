...
class MethodChannelCamera extends CameraPlatform {
  ...
  Future<int> createCamera(
    ...
    int androidAudioSource = 1,
    String iosMicrophone = "",
  }) async {
    try {
  ...
  Future<dynamic> handleDeviceMethodCall(MethodCall call) async {
    switch (call.method) {
      case 'orientation_changed':
        deviceEventStreamController.add(DeviceOrientationChangedEvent(deserializeDeviceOrientation(call.arguments['orientation']! as String)));
        break;
      default:
    ...
    switch (call.method) {
      ...
      case 'video_recorded':
        cameraEventStreamController.add(VideoRecordedEvent(
          ...
          call.arguments['maxVideoDuration'] != null ? Duration(milliseconds: call.arguments['maxVideoDuration']! as int) : null,
        ));
        break;
      case 'error':
